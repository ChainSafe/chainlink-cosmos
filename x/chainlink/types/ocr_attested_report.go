...
func (aos AttributedObservations) Median() (*Observation, error) {
	if len(aos) == 0 {
		return &Observation{}, errors.Errorf(
...
func (c *AttestedReportOne) Verify(repctx *ReportContext, a signature.OnChainSigningAddress) (err error) {
	report, err := c.AttributedObservations.OnChainReport(repctx)
	if err != nil {
		return err
	}
	var dummyID types.OracleID
	//address := map[signature.OnChainSigningAddress]types.OracleID{a: dummyID}
	address := map[signature.OnChainSigningAddress]types.OracleID{a: dummyID}
...
func (rep AttestedReportMany) Equal(c2 AttestedReportMany) bool {
	if !AttributedObservations(rep.AttributedObservations).Equal(c2.AttributedObservations) ||
...
func (rep *AttestedReportMany) TransmissionArgs(repctx *ReportContext) (report []byte, rs,
	...
	report, err = AttributedObservations(rep.AttributedObservations).OnChainReport(repctx)
	if err != nil {
